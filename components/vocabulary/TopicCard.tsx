...
export function TopicCard({ topic }: TopicCardProps) {
  ...
  const topicVocabs = useSelector((state: RootState) => {
    try {
      ...
      console.log(`TopicCard[${topic.id}]: Found ${vocabs.length} vocabs:`, vocabs);
      return vocabs;
    } catch (error) {
  ...
  const progressPercentage = vocabCount > 0 ? Math.round((knownVocabs / vocabCount) * 100) : 0;

  console.log(`TopicCard[${topic.title}] - ID: ${topic.id}, VocabCount: ${vocabCount}, Progress: ${progressPercentage}%`);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
      ...
      <div className="relative h-32 bg-gradient-to-br from-blue-500 via-purple-600 to-indigo-700 overflow-hidden">
        <img
          src={topic.coverImage || DEFAULT_TOPIC_IMAGE}
          alt={topic.title}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between bg-gray-50 rounded-lg p-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            />
            <div>
              <span className="font-bold text-gray-800 text-sm">{vocabCount}</span>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <div>
              <span className="font-bold text-gray-800 text-sm">{Math.max(15, vocabCount * 2)}</span>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-bold text-gray-800">{progressPercentage}%</span>
          </div>
